...
def reset_runstatus(db):
    ...
    db.execute_update(
        f'update craw_douyin_url set status=0 where status=1 and type="周期抓取" and create_time<date_add(now(),INTERVAL -14 day)')
    db.execute_update(
        f'update craw_douyin_url set is_run=0 where is_run=1 and type="周期抓取"  and level = "S" and status=1 and ifnull(last_runtime,updatetime)<date_add(now(),INTERVAL -10 minute)')
    db.execute_update(
        f'update craw_douyin_url set is_run=0 where is_run=1 and type="周期抓取"  and level = "A" and status=1 and ifnull(last_runtime,updatetime)<date_add(now(),INTERVAL -1 day)')
    db.execute_update(
        f'update craw_douyin_url set is_run=0 where is_run=1 and type="周期抓取"  and level = "B" and status=1 and ifnull(last_runtime,updatetime)<date_add(now(),INTERVAL -7 day)')
        ...
        while True:
            ...
            try:
                ...
                turlinfo = db.execute_query(
                    f'select * from craw_douyin_url where type="周期抓取" and (next_runtime is null or next_runtime<=now()) and is_run = 0 order by last_runtime,field(level,"S","A","B","") limit 10')
                if not turlinfo:
                    if i == 0:
                        print('没有要操作的数据')
                    time.sleep(10)
                    ...
                    for tdata in turlinfo:
                        ...
                        if rs:
                            ...
                            next_runtime = cfun.get_next_runtime(now,level,1)
                            db.update('craw_douyin_url', {
