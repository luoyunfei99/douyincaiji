    ...
    for w in range(10):
        ...
        try:
            ...
            if not isinstance(res, bool):
                ...
                print(data)
                db.update('craw_douyin_history_comment_user',data,f'id="{pid}"')
                break
            else:    
...
if __name__ == '__main__':
    ...
    while True: 
        try:
            ...
            pid = 0
            tmpinfo = None
            tmpinfo=db.execute_query(f'select * from craw_douyin_history_comment_user where status=0 limit 10')
            if not tmpinfo:
                if i==0:
                    print('没有要操作的数据')
                time.sleep(10)
                ...
                # tpage = cfun.randPage(9116,r'D:\googledata\data6',False)
                ...
                for tdata in tmpinfo:
                    ...
                    if rs:
                        #更新最后处理时间
                        db.update('craw_douyin_history_comment_user',{'status':1},f'id="{pid}"')
                    else:
                        # tpage.quit() 
                        break    
                tpage.quit()
