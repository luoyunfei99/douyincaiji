...
def getUserInfo(pid,commentinfo):
    ...
    if data.get("status_code") == 0:
        user = data["user"]
        nickname = user.get("nickname")  # 用户名
        unique_id = user.get("unique_id")  # unique_id
        signature = cfun.replace_special_chars(user.get("signature"))  # 用户介绍
        ...
        if phone and pid:
            ...
            if videoinfo:
                ...
                content = tcontent + \
                f'时间：{commentinfo["create_time"]}；昵称：{nickname}；评论文字：{commentinfo["text"]}，手机号：{phone}；品牌：{videoinfo["keyword"]}；主页链接：{url}，评论内容分级：{videoinfo["hangye_type"]}-{commentinfo["level"]}级留言'
                print(content)
                if not touser:
